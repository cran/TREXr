...
#'
#'
#' @details This dataset can be applied for testing the functions provided in \code{TREXr}.
#'
...
example.data <- function(type = "timestamp") {
  ...
  #p= processing
  #p1= data is loaded
  # proc.1 <- TREXr:::tdm.input.txt
  # read.table("tdm.input.txt", header = TRUE, sep = "\t") #p1 data is loaded {CHANGE}
  # proc.1 <- TREXr::tdm.data
  proc.1 <- getExportedValue('TREXr', 'tdm.data')
